...
sinan <-
  list.files("C:/Users/gabli/Desktop/r/SINAN/dbc",full.names = TRUE,
             ...
             pattern = ".*(14|15|16|17|18|19|20|21|22)\\.dbc$") |>
             #pattern=".dbc") |>
...
sinan |>
  ...
  tabyl(def_espec) |> arrange(desc(n)) |> head(20)
...
#Variável similar tipo de violência?
...
sinan |> tabyl(t_viol)
#Ao menos uma 18,5% e múltiplas 22,0%, ou seja, em 40% dos casos estão mal definidos.
#Se não é única, qual são as violências?

sinan |> tabyl(ano_not, t_viol)
#Dados Preliminares são problema. Olha 2022. 
...
sinan |>
  mutate(
    ...
    left_join(x= _, y = cbo, by = join_by("id_ocupa_n" == "codigo") ) -> sinan
rm(cbo)    
gc()
...
sinan |> #slice_sample(n = 1000) |>
  ...
  #ids com length inferior a seis dígitos, introduzir NA  
  across( c(id_municip, id_mn_resi, id_mn_ocor), ~
            case_when(str_length(.) < 6 ~ NA_character_,
                      .default = .) |> as_factor() ),
  
  #Acrescentar Missing nas observações com id missing
