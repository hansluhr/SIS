...
filenames <- list.files("C:/Users/gabli/Desktop/r/Sinan/dbc",full.names = TRUE,pattern=".dbc")
#Lista dos arquivos
...
#Merge em base única
sinan <- bind_rows(sinan_2009,sinan_2010,sinan_2011,sinan_2012,sinan_2013,sinan_2014,sinan_2015,sinan_2016,sinan_2017,sinan_2018,
...
sinan %>% tabyl(nu_an,rede_sau, show_na = T,show_missing_levels=T) %>% adorn_totals(where = c("row")) 
...
#Raça\cor
sinan %>% filter(ano_not == 2022 & grupo_viol!="Autoprovocada" & cs_raca!="Missing" &
...
#Sexo do autor	 
sinan %>% filter(ano_not == 2022 & grupo_viol!="Autoprovocada" & 
                   (orient_sex == "Homossexual (gay-lésbica)" | orient_sex == "Bissexual")) %>% 
...
#Situação conjugal
sinan %>% filter((ano_not == 2022 & grupo_viol!="Autoprovocada" & sit_conjug!="Missing") &
                   (orient_sex == "Homossexual (gay-lésbica)" | orient_sex == "Bissexual")) %>% 
...
sinan %>% filter(ano_not == 2022 &
  ...
  adorn_percentages(denominator = "row") %>% adorn_pct_formatting(digits = 2,affix_sign = FALSE) %>% 
...
base2021 %>% filter(id_municip != "") -> b1 #Seleciona municípios conhecidos.
base2022 %>% filter(id_municip != "") -> b2
...
base_municipios_2022 <- base2021 %>% filter(id_municip %in% vetor_municipios)

#Municípios conhecidos (Brutos)
tabyl(base2022 %>% filter(id_municip != ""),orient_sex) %>% adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior.(Mesmos Municípios)
tabyl(base_municipios_2022, orient_sex) %>% adorn_totals(where = c("row", "col"))
rm(vetor_municipios,base_municipios_2022)
...
base2021 %>% filter(id_unidade != "") -> b1 #Seleciona unidades de saúde conhecidas.
base2022 %>% filter(id_unidade != "") -> b2
vetor_unidade = intersect(b1$id_unidade,b2$id_unidade)
rm(b1,b2)
#Criando bases com unidades comuns entre os anos.
base_unidades_2022 <- base2021 %>% filter(id_unidade %in% vetor_unidade)

#Unidades conhecidas (Brutos)
tabyl(base2022 %>% filter(id_unidade != ""),orient_sex) %>% adorn_totals(where = c("row", "col"))
#Unidades iguais as do período anterior. (Mesma Unidade)
tabyl(base_unidades_2022, orient_sex) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_unidade,base_unidades_2022,base2021,base2022)
...
sinan %>% select(dt_notific,dt_ocor,id_municip,id_unidade,ident_gen,viol_fisic:viol_espec,viol_motiv,grupo_viol) %>% 
  mutate(ano = year(dt_notific), #Ano de notificação
         ano_ocor = year(dt_ocor)) %>% #Ano de ocorrência.
  filter(ano == 2022 & (ident_gen == "Transexual Homem" |ident_gen == "Transexual Mulher" | ident_gen == "Travesti") & grupo_viol!="Autoprovocada") -> base2022


#Violência Física - LGBTQI
base2021 %>% filter(viol_fisic == "Sim" & id_municip!= "") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(viol_fisic == "Sim" & id_municip!= "") ->b2
...
#Municípios conhecidos (Brutos)
tabyl(base2022 %>% filter(id_municip != "" & viol_fisic == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesmos Municípios)
tabyl(base_municipios_2022 %>% filter(viol_fisic == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_municipios,base_municipios_2022)

#Mesmo unidade - Violência Física - LGBTQI
base2021 %>% filter(viol_fisic == "Sim" & id_unidade!= "") ->b1 #Seleciona unidades com violência física.
base2022 %>% filter(viol_fisic == "Sim" & id_unidade!= "") ->b2
#x <- semi_join(base2018,base2017, by = "id_unidade", keep = TRUE)
...
base_unidades_2022 <- base2021 %>% filter(id_unidade %in% vetor_unidades)

#Unidades conhecidos (Brutos)
tabyl(base2022 %>% filter(id_unidade != "" & viol_fisic == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesma Unidade)
tabyl(base_unidades_2022 %>% filter(viol_fisic == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_unidades,base_unidades_2022)#Não apagar baseano1 e baseano2. Será usado a frente.



#Violência psicológica - LGBTQI
base2021 %>% filter(viol_psico == "Sim" & id_municip!= "") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(viol_psico == "Sim" & id_municip!= "") ->b2
#x <- semi_join(base2018,base2017, by = "id_unidade", keep = TRUE)
vetor_municipios = intersect(b1$id_municip,b2$id_municip) #Seleciona municípios que constam nas duas bases.
rm(b1,b2)
#Criando bases com municípios comuns entre os anos.
...
#Municípios conhecidos (Brutos)
tabyl(base2022 %>% filter(id_municip != "" & viol_psico == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesmos municípios)
tabyl(base_municipios_2022 %>% filter(viol_psico == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_municipios,base_municipios_2022)


#Mesma unidade - Violência psicológica - LGBTQI
base2021 %>% filter(viol_psico == "Sim" & id_unidade!= "") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(viol_psico == "Sim" & id_unidade!= "") ->b2
#x <- semi_join(base2018,base2017, by = "id_unidade", keep = TRUE)
...
tabyl(base2022 %>% filter(id_unidade != "" & viol_psico == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Unidades iguais ao do período anterior. (Memsma unidade)
tabyl(base_unidades_2022 %>% filter(viol_psico == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_unidades,base_unidades_2022)



#Tortura - LGBTQI 
base2021 %>% filter(viol_tort == "Sim" & id_municip!= "") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(viol_tort == "Sim" & id_municip!= "") ->b2
...
#Municípios conhecidos (Brutos)
tabyl(base2022 %>% filter(id_municip != "" & viol_tort == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesmos municípios)
tabyl(base_municipios_2022 %>% filter(viol_tort == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_municipios,base_municipios_2022)

#Mesmo Unidade - Tortura - LGBTQI 
base2021 %>% filter(viol_tort == "Sim" & id_unidade!= "") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(viol_tort == "Sim" & id_unidade!= "") ->b2
#x <- semi_join(base2019,base2018, by = "id_unidade", keep = TRUE)
vetor_unidades = intersect(b1$id_unidade,b2$id_unidade) #Seleciona municípios que constam nas duas bases.
rm(b1,b2)
#Criando bases com Unidades comuns entre os anos.
...
#Unidades conhecidos (Brutos)
tabyl(base2022 %>% filter(id_unidade != "" & viol_tort == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesma unidade)
tabyl(base_unidades_2022 %>% filter(viol_tort == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_unidades,base_unidades_2022)



#Outros - LGBTQI 
base2021 %>% filter(id_municip != "" & viol_outr == "Sim") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(id_municip != "" & viol_outr == "Sim") ->b2
#x <- semi_join(base2019,base2018, by = "id_unidade", keep = TRUE)
vetor_municipios = intersect(b1$id_municip,b2$id_municip) #Seleciona municípios que constam nas duas bases.
rm(b1,b2)
#Criando bases com municípios comuns entre os anos.
...
#Municípios conhecidos (Brutos)
tabyl(base2022 %>% filter(id_municip != "" & viol_outr == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Municípios iguais ao do período anterior. (Mesmos municípios)
tabyl(base_municipios_2022 %>% filter(viol_outr == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_municipios,base_municipios_2022)


##Mesmo Unidade - Outros - LGBTQI 
base2021 %>% filter(id_unidade != "" & viol_outr == "Sim") ->b1 #Seleciona municípios com violência física.
base2022 %>% filter(id_unidade != "" & viol_outr == "Sim") ->b2
#x <- semi_join(base2019,base2018, by = "id_unidade", keep = TRUE)
vetor_unidades = intersect(b1$id_unidade,b2$id_unidade) #Seleciona municípios que constam nas duas bases.
rm(b1,b2)
#Criando bases com Unidades comuns entre os anos.
...
#Unidades conhecidos (Brutos)
tabyl(base2022 %>% filter(id_unidade != "" & viol_outr == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
#Unidades iguais ao do período anterior. (Mesmas Unidades)
tabyl(base_unidades_2022 %>% filter(viol_outr == "Sim"),ident_gen) %>%  adorn_totals(where = c("row", "col"))
rm(vetor_unidades,base_unidades_2022)

rm(base2020,base2021,base2022)



# Tabelas PcD -------------------------------------------------
#Cobertura municipal das pessoas com deficiência - Municípios e CNES com ao menos uma notificação
load("C:/Users/b224552695/Desktop/SINAN/sinan_09_2022_preliminar_transtorno.RData")
...
sinan %>% filter(ano_not == 2022 & def=="Sim" & grupo_viol!="Autoprovocada") %>% 
  ...
  adorn_ns(position = "front",
           ns = tabyl(filter(sinan,ano_not == 2022 & def=="Sim" & grupo_viol!="Autoprovocada"),
...
sinan %>% filter(ano_not == 2022 & cs_sexo == "F" & def=="Sim" & grupo_viol!="Autoprovocada") %>% 
  tabyl(tipodef,grupo_viol, show_na = F, show_missing_levels = F) %>%
  adorn_totals(where = "col") %>%
  arrange(desc(Total)) %>% #Ordem das linhas
  adorn_totals(where = "row") %>%
  ...
  adorn_pct_formatting(digits = 2) %>%
  adorn_ns(position = "front",
           ns = tabyl(filter(sinan,ano_not == 2022 & cs_sexo == "F" & def=="Sim" & grupo_viol!="Autoprovocada"),
                      tipodef,grupo_viol,show_na = F, show_missing_levels = F) %>%
             adorn_totals(where = "col") %>%
             arrange(desc(Total)) %>%
             adorn_totals(where = "row")) %>% 
...
sinan %>% filter(ano_not == 2022 & cs_sexo == "M" & def=="Sim" & grupo_viol!="Autoprovocada") %>% 
  tabyl(tipodef,grupo_viol, show_na = F, show_missing_levels = F) %>%
  adorn_totals(where = "col") %>%
  arrange(desc(Total)) %>% #Ordem das linhas
  adorn_totals(where = "row") %>%
  ...
  adorn_pct_formatting(digits = 2) %>%
  adorn_ns(position = "front",
           ns = tabyl(filter(sinan,ano_not == 2022 & cs_sexo == "M" & def=="Sim" & grupo_viol!="Autoprovocada"),
                      tipodef,grupo_viol,show_na = F, show_missing_levels = F) %>%
             adorn_totals(where = "col") %>%
             arrange(desc(Total)) %>%
             adorn_totals(where = "row")) %>% 
...
sinan %>% filter(!is.na(tipodef) & cs_sexo == "M") %>%
  mutate(
    ...
    v_outro = case_when(viol_outr=="Sim" | viol_legal=="Sim" | viol_infan=="Sim" | 
                          viol_traf=="Sim" | viol_tort=="Sim" ~ 1, TRUE ~ 0),
    v_neglig = case_when(viol_negli=="Sim" ~ 1, TRUE ~ 0 )) %>%
  filter(ano_not == 2022 & def=="Sim" & grupo_viol!="Autoprovocada") %>%
  group_by(tipodef) %>% 
  summarise(V_Física = sum(v_fisica),
            V_Psicol = sum(v_psico),
            Neglig_aband = sum(v_neglig),
...
library(janitor)
#Cobertura municipal das pessoas com deficiência - Municípios e CNES com ao menos uma notificação
load("C:/Users/gabli/Desktop/r/Sinan/sinan_09_2021_preliminar_transtorno.RData")
...
sinan %>% filter(!is.na(idade) & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022)) |> 
...
sinan %>% filter(!is.na(idade) & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022) ) |> 
  mutate(fxet_eca = as.factor(case_when(
    ...
    TRUE ~ "Outras"))) |> 
  tabyl(grupo_viol,fxet_eca,show_na = F, show_missing_levels = F) |>
...
sinan |>
  filter(t_viol == "V.Física" & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022)) |> droplevels() |>
...
sinan |>
  filter(t_viol == "V.Sexual" & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022)) |> droplevels() |>
  #Criando faixa etária
  mutate(fxet_eca = as.factor(case_when(
    idade<=4 ~ "0 a 4",
    idade>=5 & idade<=14 ~ "5 a 14",
...
sinan |>
  filter(t_viol == "V.Psico" & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022)) |> droplevels() |>
  #Criando faixa etária
  mutate(fxet_eca = as.factor(case_when(
    idade<=4 ~ "0 a 4",
    idade>=5 & idade<=14 ~ "5 a 14",
...
sinan |>
  filter(t_viol == "Negligência" & grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022)) |> droplevels() |>
  #Criando faixa etária
  mutate(fxet_eca = as.factor(case_when(
    idade<=4 ~ "0 a 4",
    idade>=5 & idade<=14 ~ "5 a 14",
...
sinan |>
  ...
  filter(t_viol %in% c("Negligência","V.Física","V. Psicológica","V.Sexual") & fxet_eca != "Outras" &
           grupo_viol!="Autoprovocada" & ano_not %in% c(2012:2022) ) |> droplevels() |>
  #contagem de notificações
  count(ano_not,fxet_eca,t_viol) |> 
  ggplot(aes(x = ano_not, y = n, color = fxet_eca, group = fxet_eca) ) +
  geom_line() + geom_point() +
  ggrepel::geom_text_repel(aes(x= ano_not, y = n, label = n, color = fxet_eca), size = 3.15, show.legend = FALSE ) + 
...
ggsave(filename = "V. não letal.bmp", width = 15,height = 10,dpi=250)
ggsave(filename = "V. não letal.eps", width = 15,height = 10,dpi=350)
...
sinan |>
  ...
  mutate(fxet_eca = as.factor(case_when(
    ...
    idade>=15 & idade<=19 ~ "15 a 19",
    TRUE ~ "Outras"))) |>
  filter(grupo_viol!="Autoprovocada" & fxet_eca != "Outras" & ano_not %in% c(2012:2022) &
...
data_list <- rio::import_list("C:/Users/b224552695/Dropbox/Ipea/Atlas/Pop-PNADc-60+.xlsx", 
                              which = seq(from = 1, to = 11, by = 1), 
