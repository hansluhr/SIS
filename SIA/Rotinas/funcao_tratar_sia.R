  ...
  if (length(cols_existentes) > 0) {
    ...
    data <- dtplyr::lazy_dt(data)
      ...
      if ("PA_REGCT" %in% variables_names) {
        ...
        data <- data %>% dplyr::mutate(PA_REGCT = dplyr::case_match(.data$PA_REGCT, 
      ...
      if ("PA_INCOUT" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_INCOUT = dplyr::case_when(.data$PA_INCOUT !=
                                                                      "0000" ~ "Com incremento", .data$PA_INCOUT ==
                                                                      "0000" ~ "Sem incremento"))
      }

      if ("PA_INCURG" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_INCOUT = dplyr::case_when(.data$PA_INCURG !=
                                                                      "0000" ~ "Com incremento", .data$PA_INCURG ==
                                                                      "0000" ~ "Sem incremento"))
      }

      if ("PA_TPUPS" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_TPUPS = dplyr::case_match(.data$PA_TPUPS,
                                                                    "74" ~ "ACADEMIA DA SAÚDE", "81" ~ "CENTRAL DE REGULAÇÃO",
                                                                    ...
                                                                    "61" ~ "CENTRO DE PARTO NORMAL", "02" ~ "CENTRO DE SAUDE/UNIDADE BASICA DE SAUDE",
                                                                    "64" ~ "CENTRAL DE REGULACAO DE SERVICOS DE SAUDE",
                                                                    "36" ~ "CLINICA ESPECIALIZADA/AMBULATORIO ESPECIALIZADO",
                                                                    "22" ~ "CONSULTORIO", "60" ~ "COOPERATIVA",
                                                                    "43" ~ "FARMACIA", "07" ~ "HOSPITAL ESPECIALIZADO",
                                                                    "05" ~ "HOSPITAL GERAL", "62" ~ "HOSPITAL DIA",
                                                                    "67" ~ "LABORATORIO CENTRAL DE SAUDE PUBLICA - LACEN",
                                                                    "80" ~ "ORIO DE SAUDE PUBLICA", "04" ~ "POLICLINICA",
                                                                    "79" ~ "OFICINA ORTOPEDICA", "01" ~ "POSTO DE SAUDE",
                                                                    "73" ~ "PRONTO ANTEDIMENTO", "21" ~ "PRONTO SOCORRO ESPECIALIZADO",
                                                                    "20" ~ "PRONTO SOCORRO GERAL", "68" ~ "SECRETARIA DE SAUDE",
                                                                    "77" ~ "SERVICO DE ATENCAO DOMICILIAR ISOLADO(HOME CARE)",
                                                                    "63" ~ "UNIDADE AUTORIZADORA", "72" ~ "UNIDADE DE ATENÇÃO À SAÚDE INDÍGENA",
                                                                    "78" ~ "UNIDADE DE ATENCAO EM REGIME RESIDENCIAL",
                                                                    "39" ~ "UNIDADE DE SERVICO DE APOIO DE DIAGNOSE E TERAPIA",
                                                                    "45" ~ "UNIDADE DE SAUDE DA FAMILIA", "50" ~
                                                                      "UNIDADE DE VIGILANCIA EM SAUDE", "65" ~ "UNIDADE DE VIGILANCIA EPIDEMIOLOGIA (ANTIGO)",
                                                                    "66" ~ "UNIDADE DE VIGILANCIA SANITARIA (ANTIGO)",
                                                                    "15" ~ "UNIDADE MISTA", "42" ~ "UNIDADE MOVEL DE NIVEL PRE-HOSP-URGENCIA/EMERGENCIA",
                                                                    "32" ~ "UNIDADE MOVEL FLUVIAL", "40" ~ "UNIDADE MOVEL TERRESTRE",
                                                                    "75" ~ "TELESAÚDE", "09" ~ "PRONTO SOCORRO DE HOSPITAL GERAL (ANTIGO)",
      ...
      if ("PA_TIPPRE" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_TIPPRE = dplyr::case_match(.data$PA_TIPPRE,
                                                                     "20" ~ "PRIVADO COM FINS LUCRATIVOS", "22" ~
                                                                       "PRIVADO OPTANTE PELO SIMPLES", "30" ~ "PUBLICO FEDERAL",
                                                                     "40" ~ "PUBLICO ESTADUAL", "50" ~ "PUBLICO MUNICIPAL",
                                                                     "60" ~ "PRIVADO SEM FINS LUCRATIVOS", "61" ~
                                                                       "FILANTROPICO COM CNAS VALIDO", "80" ~ "SINDICATO",
                                                                     .default = .data$PA_TIPPRE)) %>% dplyr::mutate(PA_TIPPRE = as.factor(.data$PA_TIPPRE))
      }

      if ("PA_MN_IND" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_MN_IND = dplyr::case_match(.data$PA_MN_IND,
                                                                     "M" ~ "Mantida", "I" ~ "Individual", .default = .data$PA_MN_IND))
      }
      ...
      if ("PA_TPFIN" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_TPFIN = dplyr::case_match(.data$PA_TPFIN,
                                                                    "01" ~ "Atenção Básica (PAB)", "02" ~ "Assistência Farmacêutica",
                                                                    "04" ~ "Fundo de Ações Estratégicas e Compensações FAEC",
                                                                    "05" ~ "Incentivo - MAC", "06" ~ "Média e Alta Complexidade (MAC)",
                                                                    "07" ~ "Vigilância em Saúde", .default = .data$PA_TPFIN)) %>%
          dplyr::mutate(PA_TPFIN = as.factor(.data$PA_TPFIN))
      }

      if ("PA_NIVCPL" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_NIVCPL = dplyr::case_match(.data$PA_NIVCPL,
                                                                     "0" ~ "Não se Aplica", "1" ~ "Atenção Básica",
      ...
      if ("PA_DOCORIG" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_DOCORIG = dplyr::case_match(.data$PA_DOCORIG,
                                                                      "C" ~ "BPA-C", "I" ~ "BPA-I", "P" ~ "APAC - Procedimento Principal",
                                                                      "S" ~ "APAC - Procedimento Secundário", "A" ~
                                                                        "RAAS - Atenção Domiciliar", "B" ~ "RAAS - Psicossocial",
                                                                      .default = .data$PA_DOCORIG)) %>% dplyr::mutate(PA_DOCORIG = as.factor(.data$PA_DOCORIG))
      }
      ...
      if ("PA_MOTSAI" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_MOTSAI = dplyr::case_match(.data$PA_MOTSAI,
                                                                     "11" ~ "ALTA CURADO", "12" ~ "ALTA MELHORADO",
                                                                     "13" ~ "ALTA DA PUÉRPERA E PERMANÊNCIA DO RECÉM NASCIDO",
                                                                     "14" ~ "ALTA A PEDIDO", "15" ~ "ALTA COM PREVISÃO DE RETORNO P/ ACOMPAN. DO PACIENT",
                                                                     "16" ~ "ALTA POR EVASÃO", "17" ~ "ALTA DA PUÉRPERA E RECÉM NASCIDO",
                                                                     "18" ~ "ALTA POR OUTROS MOTIVOS", "21" ~ "PERMANÊNCIA POR CARACTERÍSTICAS PRÓPRIAS DA DOENÇA",
                                                                     "22" ~ "PERMANÊNCIA POR INTERCORRÊNCIA", "23" ~
                                                                     ...
                                                                     "27" ~ "PERMANÊNCIA POR REOPERAÇÃO", "28" ~
                                                                       "PERMANÊNCIA POR OUTROS MOTIVOS", "31" ~
                                                                     ...
                                                                     "51" ~ "ENCERRAMENTO ADMINSTRATIVO", "00" ~
      ...
      if ("PA_OBITO" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_OBITO = dplyr::case_match(.data$PA_OBITO,
                                                                    "1" ~ "Sim (motivo de saída-ÓBITO)", "0" ~
                                                                      "Nao houve ÓBITO", .default = .data$PA_OBITO)) %>%
          dplyr::mutate(PA_OBITO = as.factor(.data$PA_OBITO))
      }

      if ("PA_ENCERR" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_ENCERR = dplyr::case_match(.data$PA_ENCERR,
                                                                     "1" ~ "Sim (motivo de saída-ENCERRAMENTO)",
                                                                     "0" ~ "Nao houve ENCERRAMENTO", .default = .data$PA_ENCERR)) %>%
          dplyr::mutate(PA_ENCERR = as.factor(.data$PA_ENCERR))
      }

      if ("PA_PERMAN" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_PERMAN = dplyr::case_match(.data$PA_PERMAN,
      ...
      if ("PA_ALTA" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_ALTA = dplyr::case_match(.data$PA_ALTA,
                                                                   "1" ~ "Sim (motivo de saída-PERMANÊNCIA)",
                                                                   "0" ~ "Nao houve a PERMANÊNCIA do paciente na unidade",
      ...
      if ("PA_TRANSF" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_TRANSF = dplyr::case_match(.data$PA_TRANSF,
      ...
      if ("PA_CATEND" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_CATEND = dplyr::case_match(.data$PA_CATEND,
                                                                     "01" ~ "ELETIVO", "02" ~ "URGÊNCIA", "03" ~
                                                                       "ACIDENTE NO LOCAL TRABALHO OU A SERViÇO DA EMPRESA",
                                                                     ...
                                                                     "07" ~ "CARATER DE ATENDIMENTO INVALIDO", "10" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", "12" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", "20" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", "53" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", "54" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", "57" ~
                                                                       "CARATER DE ATENDIMENTO INVALIDO", .default = .data$PA_CATEND)) %>%
      ...
      if ("PA_FLIDADE" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_FLIDADE = dplyr::case_match(.data$PA_FLIDADE,
                                                                      "0" ~ "IDADE NÃO EXIGIDA", "1" ~ "IDADE COMPATIVEL COM O SIGTAP",
                                                                      "2" ~ "IDADE FORA DA FAIXA DO SIGTAP", "3" ~
                                                                        "IDADE INEXISTENTE", "4" ~ "IDADE EM BRANCO",
                                                                      .default = .data$PA_FLIDADE)) %>% dplyr::mutate(as.factor(.data$PA_FLIDADE))
      }

      if ("PA_SEXO" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_SEXO = dplyr::case_match(.data$PA_SEXO,
                                                                   "0" ~ "Não exigido", "M" ~ "Masculino", "F" ~
                                                                     "Feminino", .default = .data$PA_SEXO)) %>%
          dplyr::mutate(as.factor(.data$PA_SEXO))
      }

      if ("PA_RACACOR" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_RACACOR = dplyr::case_match(.data$PA_RACACOR,
                                                                      "00" ~ "RAÇA/COR NÃO EXIGIDO", "01" ~ "BRANCA",
                                                                      "02" ~ "PRETA", "03" ~ "PARDA", "04" ~ "AMARELA",
                                                                      "05" ~ "INDIGENA", "99" ~ "SEM INFORMAÇÃO",
                                                                      "06" ~ "RAÇA/COR=06 (INDEVIDO)", "09" ~ "RAÇA/COR=09 (INDEVIDO)",
                                                                      "1M" ~ "RAÇA/COR  (OUTROS INDEVIDOS)", "1G" ~
                                                                        "RAÇA/COR  (OUTROS INDEVIDOS)", "1C" ~ "RAÇA/COR  (OUTROS INDEVIDOS)",
                                                                      "DE" ~ "RAÇA/COR  (OUTROS INDEVIDOS)", "D" ~
                                                                        "RAÇA/COR  (OUTROS INDEVIDOS)", "87" ~ "RAÇA/COR  (OUTROS INDEVIDOS)",
      ...
      if ("PA_UFDIF" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_UFDIF = dplyr::case_match(.data$PA_UFDIF,
                                                                    "1" ~ "Sim (houve invasão)", "0" ~ "Não houve invasão",
                                                                    .default = .data$PA_UFDIF)) %>% dplyr::mutate(as.factor(.data$PA_UFDIF))
      }
      if ("PA_MNDIF" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_MNDIF = dplyr::case_match(.data$PA_MNDIF,
                                                                    "1" ~ "Sim (houve invasão)", "0" ~ "Não houve invasão",
                                                                    .default = .data$PA_MNDIF)) %>% dplyr::mutate(as.factor(.data$PA_MNDIF))
      }
      ...
      if ("PA_INDICA" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_INDICA = dplyr::case_match(.data$PA_INDICA,
                                                                     "5" ~ "Aprovado totalmente", "6" ~ "Aprovado parcialmente",
                                                                     "0" ~ "Não aprovado", .default = .data$PA_INDICA)) %>%
          dplyr::mutate(as.factor(.data$PA_INDICA))
      }

      if ("PA_ETNIA" %in% variables_names) {
        data <- data %>% dplyr::mutate(PA_ETNIA = dplyr::case_match(.data$PA_ETNIA,
                                                                    ...
                                                                    "X402" ~ "ARARA", "X403" ~ "SAKIRIABAR", "X404" ~
                                                                      "TATZ", "X405" ~ "SEM INFORMACAO", .default = .data$PA_ETNIA)) %>%
