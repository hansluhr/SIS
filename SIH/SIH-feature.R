...
executar_consulta <- function(tabela) {
  ...
  con <- dbConnect(drv, dbname = "sis_datasus", host = "psql13rj-saude.ipea.gov.br", 
                   port = 5432, user = "p224552695", password= "Do355ju290*")
  ...
  query <- sprintf("
    ...
    WHERE CAST(ident AS INTEGER) = 1 
      AND CAST(ano_cmpt AS INTEGER) = 2023
...
library(janitor)
#Importando a base
load("C:/Users/b224552695/Desktop/r/SIH/sih_18_24.RData")

#Essa base veio do dbeaver
...
#Descartar causa_letra e causa_num. Não utilizados
sih |> select(!c(causa_letra, causa_num ) ) -> sih

#Rename das variáveis de interesse
sih |>
  rename(
    #Escolaridade
    esc = instru,
    ...
    dt_nasc = nasc,
    #local do incidente
    local_incd =  local_obito) -> sih

#Instrumento
sih %>% mutate(instrumento = replace_na(instrumento,"Desconhecido")) -> sih
...
sih |> 
  ...
  ano_cmpt = ano_cmpt |> as_factor(),
  mes_cmpt = mes_cmpt |> as_factor(), 
  dtm_cmpt = zoo::as.yearmon(paste(ano_cmpt,mes_cmpt), "%Y %m" ), 
  ...
  dtm_inter = zoo::as.yearmon(dt_inter,"%Y%b"), #Data da internação mensal.
  ...
  dtm_saida = zoo::as.yearmon(dt_saida,"%Y%b"), 
   ...
   #Escolaridade
   #A variável escolaridade apresenta elevado Missing. Quase totalidade de preenchimento 0. Esse valor não está no dicionário.  
   esc = case_match(esc, "1" ~ "Analfabeto", "2" ~ "1º Grau", "3" ~ "2º Grau", "4" ~ "3º Grau", .default = "Missing") |> as_factor(),
  
   #Morte
   morte = case_match(morte, 0 ~ "Não", 1 ~ "Sim"),
   
   #Local do incidente. Elevado mal preenchimento.
  local_incd = case_match(local_incd, 0 ~ "Residencial", 1 ~ "Hab, Coletiva", 2 ~ "Área de administração pública*",
                           3 ~  "Esportiva", 4 ~ "Rua/Estrada", 5 ~ "Comercial", 6 ~ "Industrial",  
                           7 ~ "Fazenda", 8 ~ "Outros", 9  ~ "Ignorado", .default = "Missing"),
  #Local de óbito de intervenção legal é rua/estrada
  local_incd = case_when(intencao == "h_legal" ~ "Rua/Estrada", TRUE ~ local_incd) ) -> sih


### UF e município de residência
#Importação municípios e UF de residência.
#Fonte: https://www.ibge.gov.br/explica/codigos-dos-municipios.php
base_uf_munic <- readxl::read_excel("C:/Users/b224552695/Dropbox/Ipea/Atlas/municipios_br.xls",sheet = "munics") |>
  ...
  #Microdados do SIH contém 6 dígitos no cod de município. 
...
#Join de municípios e UFs
#Vou adicionar label do município e UF de residência através da base_munics
sih |>
  #Join do label da UF e município de residência
  left_join(x=_, y = base_uf_munic, by = join_by("munic_res" == "cod_ibge") ) -> sih
rm(base_uf_munic)
...
base_uf_munic <- readxl::read_excel("C:/Users/b224552695/Dropbox/Ipea/Atlas/municipios_br.xls",sheet = "munics") |>
  select("Nome_UF", "Código Município Completo","Nome_Município") |> clean_names() |>
  rename(cod_ibge = "codigo_municipio_completo", munic_inter = nome_municipio,uf_inter = nome_uf) |>
  #Microdados do SIH contém 6 dígitos no cod de município. 
  #Vou deixar todos os municípios em todos os anos com 6 dígitos.
  mutate(cod_ibge = substr(cod_ibge,1,6),
         #Substituição Brasília Por DF
         munic_inter = case_when(munic_inter == "Brasília" ~ "Distrito Federal", .default = munic_inter) ) 

#Join de municípios e UFs
#Vou adicionar label do município e UF de residência através da base_munics
sih |>
  #Join do label da UF e município de residência
  left_join(x=_, y = base_uf_munic, by = join_by("munic_mov" == "cod_ibge") ) -> sih
rm(base_uf_munic)


###Atribuindo Etnia
#Importando etnia
etn <- readxl::read_excel("C:/Users/b224552695/Dropbox/Ipea/Atlas/Dicionário/SIH/bases_auxiliares/ETNIA.xlsx") |>
...
sih |>
  mutate(
  #Removendo o zeros a esquerda da variável cod_etn."^0+"
  cod_etn = str_remove(cod_etn, "^0+(?=\\d)") |> as_factor() ) |> 
  #Join para pegar label das etnias.
...
# Check
sih |> 
  tabyl(etnia,raca_cor) |> view()
#Etina está preenchido somente para raca\cor indígena.
...
sih |> 
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |> 
  tabyl(uf_resd,ano_inter) |> adorn_totals(where = c("col","row"))
...
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |> 
  tabyl(etnia, ano_inter) |> adorn_totals(where = c("row","col") ) |>
  #Exclusão de etnia sem internações
  filter(Total > 0) |>
  view()
...
sih |>
  ...
  adorn_totals(where = c("col","row") ) |> view()
  
#Homicídio por ano de internaçaõ e Município de residência
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |>
...
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |>
  tabyl(instrumento) |> adorn_pct_formatting() |> arrange(desc(n))


#local do incidente
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |>
  tabyl(local_incd) |> adorn_pct_formatting() |> arrange(desc(n))

#sexo do indígena
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |>
  tabyl(sexo) |> adorn_pct_formatting() 

#Houve óbito?
sih |>
  filter(intencao %in% c("Homicídio", "h_legal") & raca_cor == "Indígena") |>
  tabyl(morte) |> adorn_pct_formatting() 
...
sih |>
  ...
  filter(!is.na(idade)) |>
  ggplot() +
  geom_histogram(aes(x=idade), binwidth  = 1) +
  #geom_vline(aes(xintercept = mean(idade)), color = "red",linewidth = 1, linetype="dashed")  +
...
#Histograma de idade
sih |>
  filter(intencao %in% c("Homicídio", "h_legal")) |>
  ggplot(aes(x=idade)) +
  stat_ecdf( aes(color = raca_cor),  pad = FALSE,na.rm=TRUE) + geom_hline(yintercept=0.5,linetype = "dotted") +
  labs(x = "", y="",color = "") +
  scale_x_continuous(breaks = seq(0, 110, 5)) + scale_y_continuous(breaks = seq(0, 10, 0.1), labels = scales::percent) + 
  theme(legend.position = c(0.75,0.2),legend.background = element_blank(),legend.direction = "horizontal") 
